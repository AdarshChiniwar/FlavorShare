...
{
    public class AddReciepsViewModel : BaseViewModel, IRefreshable
    {
        #region Properties
        ...
        public ICommand AddCompleteReciepeCmd { get; set; }
        #endregion



        #region Constructor
        public AddReciepsViewModel()
        {
            AddIngredientCommand = new Command(AddIngredients);
            AddCompleteReciepeCmd = new Command(AddCompeteReceipe);
        ...
        #region Functions
        private async void AddCompeteReceipe()
        {
           try
            {
                ...
                else
                {
                    ...
                    //}
                    int result = sqliteConnection.Insert(foodReceipe);
                    if (result == 1)
                    {
                        ...
                        await Application.Current.MainPage.DisplayAlert("Info", "Congratulations , Reciepe added successfully", "OK");

                    }
                    else
                    {
                        await Application.Current.MainPage.DisplayAlert("Error", "Please try again", "ERROR");
                    }
                }
            }
            catch (Exception ex)
